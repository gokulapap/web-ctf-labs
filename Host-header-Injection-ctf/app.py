...
def register():
	if request.cookies.get("site_access") == "yes":
		return render_template("register.html", info="Register with same creds in home page")
	else:
		return '''
...
def dashboard():
	if request.method == "POST":
		...
		if user == "admin@wctf.com" and pas == "secret_password":
...
def recover():
	if request.method == "POST":
		if request.form['email'] == 'admin@wctf.com':
